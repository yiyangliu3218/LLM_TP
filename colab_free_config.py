...
"""
Colab免费模型配置
使用sentence_transformers等适合话题建模的免费模型
"""
...
def create_colab_config():
    ...
    config = {
        ...
        'model_name': 'all-MiniLM-L6-v2',  # sentence_transformers模型
        'num_topics': 8,
        'max_iterations': 3,
        'sample_size': 1000,
        'temperature': 0.7,
        ...
        'huggingface_config': {
            ...
            'models': {
                ...
                'generation_model': 'microsoft/DialoGPT-medium',  # 文本生成
                'classification_model': 'distilbert-base-uncased'  # 分类
            },
            'use_local': False,  # Colab上使用在线模型
            'max_length': 512,
            'batch_size': 32
        },
...
def main():
    ...
    # 2. 保存配置文件
    with open('colab_config.json', 'w', encoding='utf-8') as f:
        json.dump(config, f, ensure_ascii=False, indent=2)
    
    # 3. 生成安装命令
    install_commands = install_colab_dependencies()
    
    # 4. 生成笔记本代码
    notebook_code = create_colab_notebook()
    
    # 5. 保存所有文件
    with open('colab_install_commands.txt', 'w', encoding='utf-8') as f:
    ...
    print()
    print("📁 生成的文件:")
    print("   - colab_config.json: Colab专用配置")
    print("   - colab_install_commands.txt: 安装命令")
    print("   - colab_notebook.py: 完整笔记本代码")
    print()
    print("🚀 在Colab中使用:")
    print("   1. 上传这些文件到Colab")
    print("   2. 运行 colab_notebook.py 中的代码")
    print("   3. 或者直接复制代码到Colab笔记本")
    print()
    print("💡 使用的免费模型:")
    print("   - sentence-transformers/all-MiniLM-L6-v2 (文本嵌入)")
    print("   - microsoft/DialoGPT-medium (文本生成)")
    print("   - distilbert-base-uncased (分类)")
    print()
    print("🎉 完全免费，无需API密钥！")
